...
| getName()          | 获取当前线程的名字                              |                                                                                                                                                       |
| currentThread()    | 获取当前线程对象, 代码在哪个线程中执行          | 静态方法 & 本地方法                                                                                                                                   |
| sleep(long)        | 让当前线程休眠多少秒再继续执行                  | 静态方法 & 本地方法 <br> `Thread.sleep(0)` 让操作系统立刻重新进行一次 CPU 竞争                                                                        |
| yeild()            | 提示当前线程调度器尽量让出当前线程对 CPU 的占用 | 静态方法 & 本地方法 <br> 调用该方法并不能保证一定会让出 CPU 资源                                                                                      |
...
- 这个写回内存的操作会引起在其他 CPU 里缓存了该内存地址的数据无效(MESI 协议)
